...
if (DATABASE_TYPE === 'pdo_mysql' || DATABASE_TYPE === 'mysql') {
    ...
    db_query('
        CREATE TABLE IF NOT EXISTS ' . DATABASE_PREFIX . 'users(
            ...
            email_activated TINYINT(1) UNSIGNED NOT NULL                COMMENT \'メールアドレス認証\',
            loggedin        DATETIME                                    COMMENT \'最終ログイン日時\',
            failed          INT UNSIGNED                                COMMENT \'ログイン失敗回数\',
            failed_last     DATETIME                                    COMMENT \'最終ログイン失敗日時\',
            token           VARCHAR(255)                                COMMENT \'認証用のトークン\',
            token_code      VARCHAR(80)                                 COMMENT \'認証用のコード\',
            token_expire    DATETIME                                    COMMENT \'認証用のURL有効期限\',
    ...
    db_query('
        CREATE TABLE IF NOT EXISTS ' . DATABASE_PREFIX . 'users(
            ...
            email_activated BOOLEAN       NOT NULL,
            loggedin        TIMESTAMP,
            failed          INTEGER,
    ...
    db_query('
        CREATE TABLE IF NOT EXISTS ' . DATABASE_PREFIX . 'users(
            ...
            email_activated INTEGER UNSIGNED NOT NULL,
            loggedin        DATETIME,
